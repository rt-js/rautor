import { compile_state_init, compile_state_keys, compile_state_result, type CompileCallback } from './compiler';
...
export default class Matcher<T> {
  ...
  public compile(): MatchCallback<T> {
    const values: any[] = [];
    const state = compile_state_init(compile, values);

    const tree = this.tree;
    tree_compile(tree, state);
    // eslint-disable-next-line
    return Function(...compile_state_keys(values), `return (p)=>{${compile_state_result(state)};return null;}`)(...values) as MatchCallback<T>;
  }
}
